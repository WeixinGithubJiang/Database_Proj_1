...
begin 
 ...
 set g = (select count(*) from uosoffering where UoSCode = c);
 if e = 0 then 
  if g = 0 then
   set result = 7;
  elseif a = 0 and b = 1 then 
   ...
   set result = 1;
		...
		elseif a > 1 and b = 1 then
			set f = (select count(*) from requires s left outer join transcript t on (s.PrereqUoSCode = t.UoSCode) where s.UoSCode = c and StudId = id and (grade = 'CR' or grade = 'P' or grade = 'D'));
			if a = f then
				insert into transcript values(id, c, s, y, NULL);
				update uosoffering set Enrollment = Enrollment + 1 where UoSCode = c and Semester = s and Year = y;
				set result = 1;
