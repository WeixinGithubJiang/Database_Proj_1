...
create definer=`root`@`localhost` procedure `enroll`(in c char(20), in s1 char(10), in s2 char(10), in y char(10), in id char(10), out result integer, out req char(20))
begin 
	declare a integer;
	declare b integer;
	declare d char(20);
	...
	set b = (select count(*) from uosoffering where UoSCode = c and (Semester = s1 or Semester = s2) and Year = y and Enrollment < MaxEnrollment);
	set e = (select count(*) from transcript where UoSCode = c and StudId = id);
	if e = 0 then 
		if a = 0 and b = 1 then 
			insert into transcript values(id, c, s, y, NULL);
			update uosoffering set Enrollment = Enrollment + 1 where UoSCode = c and (Semester = s1 or Semester = s2) and Year = y;
			set result = 1;
...
delimiter //
create trigger cal_enroll_percent before update on uosoffering
for each row 
begin
	set enroll_percent = new.Enrollment / new.MaxEnrollment;
end; //